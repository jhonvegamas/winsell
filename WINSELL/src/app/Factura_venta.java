...
public class Factura_venta extends JFrame {
	...
	Statement sentenciacli,sentenciaven,sentenciacab,sentenciadet,sentenciaart,sentenciaxreg,sentenciaupdate,sentenciastock;  //  @jve:decl-index=0:
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					try{
						...
						if (respuesta==0){
							guacan();
							nuevafac=false;
							sentenciacab.executeQuery("SELECT *,(sub_gen-tot_des+tot_iva) as total,round((sub_gen-tot_des+tot_iva-pago_ini),2) as saldo FROM FACV_CAB ORDER BY NUM_FACV");
							 resultadocab=sentenciacab.getResultSet();
	...
	private JButton getBModifica() {
		if (BModifica == null) {
			...
			BModifica.setIcon(new ImageIcon("/IMAGENES/MODIFICAR.JPG"));
						...
						if (JTable.isCellSelected(JTable.getSelectedRow(), 0)==false){
							...
							if (f_lineadetalle.ingresadet==true)
							   {long xcant=Long.parseLong(f_lineadetalle.cantidad);
								actualizarticulos();
							    resultadoart.first();
							    sentenciaxreg.executeQuery("select count(id_art) as cuantos from articulos");
...
//				SubstanceDefaultLookAndFeel.setCurrentWatermark("org.jvnet.substance.watermark.SubstanceMarbleVeinWatermark");//fondo por defecto
	...
	private void initialize() {
		...
		this.setIconImage(Toolkit.getDefaultToolkit().getImage("IMAGENES/VENTAS.JPG"));
	...
	public void conectar(){
	    ...
	    {Class.forName("com.mysql.jdbc.Driver").newInstance();
		 ...
		 sentenciacli=conexion.createStatement();
		 sentenciacli.executeQuery("select * from clientes order by id_cli");
		 resultadocli=sentenciacli.getResultSet();
		 sentenciaven=conexion.createStatement();
		 sentenciaven.executeQuery("select id_ven, concat(ape_ven,' ',nom_ven) as elvendedor from vendedores order by elvendedor");
		 resultadoven=sentenciaven.getResultSet();
		 cargarvendedores();
		 sentenciaart=conexion.createStatement();
		 actualizarticulos();
		 sentenciacab=conexion.createStatement();
		 sentenciacab.executeQuery("SELECT *,(sub_gen-tot_des+tot_iva) as total,round((sub_gen-tot_des+tot_iva-pago_ini),2) as saldo FROM FACV_CAB ORDER BY NUM_FACV");
		 resultadocab=sentenciacab.getResultSet();
		 if (resultadocab.next()==true){
			 xfacturas=1;
	...
	public void localizacliente(){
		try{
			...
			sentenciaxreg.executeQuery("select count(id_cli) as cuantos from clientes");
			resultadoxreg=sentenciaxreg.getResultSet();
			resultadoxreg.last();
			resultadocli.first();
			for (int xreg=1;xreg<=resultadoxreg.getInt("cuantos");xreg++){
	...
	public void actualizarticulos(){
		try{
			sentenciaart.executeQuery("SELECT ID_ART,DES_ART,STOCK,PVP_ART FROM ARTICULOS ORDER BY ID_ART");
			resultadoart=sentenciaart.getResultSet();
			resultadoart.last();
		}
		catch(Exception s){JOptionPane.showMessageDialog(null,"Noooooooo");}
	}
	public void ingresarticulo(){
		...
		catch(Exception s){JOptionPane.showMessageDialog(null,"Error al cargar Articulo");}
	}
...
//	}
	...
	public void cargarfactura(){
		...
		catch(Exception s){JOptionPane.showMessageDialog(null,"error carga");}
	}
	...
	public void cargardetalle(){
		try{sentenciadet=conexion.createStatement();
		    limpiardetalle();
			sentenciadet.executeQuery("select facv_det.id_art,articulos.des_art,facv_det.pvp,facv_det.cant from facv_det inner join articulos on(facv_det.id_art=articulos.id_art) where num_facv='"+resultadocab.getString("num_facv")+"'");
		    resultadodet=sentenciadet.getResultSet();
		    resultadodet.first();
    ...
    private void localizafactura(){
    	try{sentenciacab.executeQuery("SELECT *,(sub_gen-tot_des+tot_iva) as total,round((sub_gen-tot_des+tot_iva-pago_ini),2) as saldo FROM FACV_CAB ORDER BY NUM_FACV");
			resultadocab=sentenciacab.getResultSet();
    		resultadocab.first();
    ...
    private void guardar(){
    	try{int xfac=modelo.getRowCount();
    	    boolean control=true;
    	    sentenciastock=conexion.createStatement();
    		for (int x=0;x<xfac;x++)
    	    {   String codlinea=new String (JTable.getValueAt(x,0).toString());
    			sentenciastock.executeQuery("SELECT STOCK FROM ARTICULOS WHERE ID_ART='"+codlinea+"'");
    	        resultadostock=sentenciastock.getResultSet();
