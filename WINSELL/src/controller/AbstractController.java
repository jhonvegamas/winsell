...
//     public int eliminacionTemporalArt(String nombreTabla, String nomColumnaCambiar, String nomColumnaId, String id, int nuevoValor) {
//        int resultado = 1;
//        if (nuevoValor < 0 || nuevoValor > 1) {
//            throw new ExceptionValoresNoIgual("el valor no puede ser menor de 0 ni mayor de 1");
//        }
...
//        
//        }
//        String sql = "DELETE FROM " + nombreTabla + " where " + nomColumnaId + "='" + id + "'";
//        try {
//            resultado = BaseConexion.getStatement().executeUpdate(sql);
//
//        } catch (SQLException ex) {
...
//    }
     ...
     * @param columnaId nombre de la columna. que serivara como base para
     * ordenar los regstros<br/>
     ...
     * si no se desa especificar este campo entonces se debe pasar
     * <code>null</code>
     * </p>
     * @param valorCondicion el valor que servira como condicion para la columna
     * condicional.<br/>
     * <p>
     * Este valor solo debe especificarse, si nombre de la columna condicional
     * es diferente de <code>null</code></p>
     * @param inicio entero que indica desde donde se comensara la obtencion de
     * registros.
     * @param fin numero de registros;
     * @param asc true para que los registros se ordenen de forma
     * ascendente.<br/>
     * <code>false</code> para ordenar los registros se ordenen de forma
     * descendente<br/>
     * ejemplo completo consideranto que controller es clase hija de
     * <code>AbstractController</code>:<br/>
     * si se llama a este metodo de la siguiente forma:<br/>
     * <code>Integer valorCondicional = new Integer(1)</code>; valor condicional
     * es igual a 1<br/>
     ...
     * select * from cliente where activo = valorCondicional order by idCliente
     * asc;<br/>
    ...
    public void getPaginador(String nombreTabla, String columnaId, String columnCondicion, Object valorCondicion, int inicio, int fin, boolean asc) {
     ...
     public void getPaginadorArt(String nombreTabla, String columnaId, String columnCondicion, Object valorCondicion, int inicio, int fin, boolean asc) {

        String sql = "SELECT * FROM " + nombreTabla + " " + (columnCondicion != null ? "" : "") + " " + (asc ? "ORDER BY " + columnaId + " asc" : "ORDER BY " + columnaId + " desc") + " LIMIT " + inicio + "," + fin;
        System.out.println(sql);
        try {
            rs = BaseConexion.getStatement().executeQuery(sql);
        } catch (SQLException ex) {
    ...
    public int geNumRegistros(String tblName, String nomCampoCondicional, Object valorCondicion, boolean todos) {
     ...
     public int geNumRegistrosArt(String tblName, String nomCampoCondicional, Object valorCondicion, boolean todos) {
        String sql = "SELECT COUNT(" + nomCampoCondicional + ") AS num FROM " + tblName + " " + (!todos ? "" + valorCondicion.toString() : "");
